...
class WPSH_Core
{
    ...
    {
      $shamsi = WPSH_PLUGINS_PATH . '/wp-shamsi-pro/wp-shamsi-pro.php';
      $exists = file_exists($shamsi) ? true : false;

      if (in_array('wp-shamsi-pro/wp-shamsi-pro.php', apply_filters('active_plugins', get_option('active_plugins'))) && $exists) {
          return true;
        } else {
            return false;
        }
    ...
    {
        if ($mode == 'string') {
            $get = (isset($_GET[$key])) ? wp_kses_post($_GET[$key]) : null;
        }
    ...
    {
        if ($mode == 'string') {
            $post = (isset($_POST[$key])) ? wp_kses_post($_POST[$key]) : null;
        }
